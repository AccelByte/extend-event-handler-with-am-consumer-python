...
app written in `Python`. It includes examples to handle AGS `userLoggedIn` and `userThirdPartyLoggedIn` events
...
├── src
│  └── app
│    ├── proto                            # AGS event spec files (*.proto)
│    └── services
│      ├── login_handler.py               # userLoggedIn event handler
│      └── third_party_login_handler.py   # userThirdPartyLoggedIn event handler
...
```
